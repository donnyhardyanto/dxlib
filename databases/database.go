...
func (d *DXDatabase) CheckConnection() (err error) {
	...
	if err != nil {
		d.Connected = false
		return errors.Wrapf(err, "databases %v CheckConnection() failed", d.NameId)
	}
	defer func() {
		_ = dbConn.Close()
	}()
...
func (d *DXDatabase) ApplyFromConfiguration() (err error) {
	if !d.IsConfigured {
		...
		if !ok {
			if d.MustConnected {
				return errors.Errorf("databases %s configuration not found", d.NameId)
			} else {
				return errors.Errorf("manager is unusable, databases %s configuration not found", d.NameId)
			}
		}
		...
		if !ok {
			if d.MustConnected {
				return errors.Errorf("mandatory database_type field value in databases %s configuration is not supported (%v)", d.NameId, s)
			} else {
				return errors.Errorf("configuration is unusable, mandatory database_type field value databases %s configuration  is not supported (%v)", d.NameId, s)
			}
		}
		d.DatabaseType = base.StringToDXDatabaseType(s)
		if !d.DatabaseType.IsValid() {
			if d.MustConnected {
				return errors.Errorf("mandatory value of database_type field of Database %s configuration is not supported (%s)", d.NameId, s)
			} else {
				return errors.Errorf("configuration is unusable, value of database_type field of databases %s configuration is not supported (%s)", d.NameId, s)
			}
		}
...
func (d *DXDatabase) CheckIsErrorBecauseDbNotExist(err error) bool {
	...
	switch d.DatabaseType {
	case base.DXDatabaseTypePostgreSQL:
		t1 := strings.Contains(s, "databases")
...
func (d *DXDatabase) Connect() (err error) {
	if !d.Connected {
		...
		if err != nil {
			...
			if d.MustConnected {
				log.Log.Fatalf("Cannot connect and ping to databases %s/%s (%s)", d.NameId, d.NonSensitiveConnectionString, err.Error())
				return nil
			} else {
				return errors.Wrapf(err, "Cannot connect and ping to databases %s/%s", d.NameId, d.NonSensitiveConnectionString)
			}
		}
		d.Connected = true
		log.Log.Infof("Connecting to databases %s/%s... done CONNECTED", d.NameId, d.NonSensitiveConnectionString)
	}
	return nil
}
...
func (d *DXDatabase) Disconnect() (err error) {
	if d.Connected {
		log.Log.Infof("Disconnecting to databases %s/%s... start", d.NameId, d.NonSensitiveConnectionString)
...
func (d *DXDatabase) ExecuteSQLContent(content string) (r sql.Result, err error) {
	...
	defer func() {
		if err != nil {
			_ = errors.Wrapf(err, "Error executing SQL content")
		}
	}()
...
func (d *DXDatabase) Tx(ctx context.Context, log *log.DXLog, isolationLevel sql.IsolationLevel, callback DXDatabaseTxCallback) (err error) {
	...
	case "oracle":
		...
		err = tx.Commit()
		if err != nil {
			log.Errorf(err, "TX_ERROR_IN_COMMIT:Details:%+v", err)
			errTx := tx.Rollback()
			if errTx != nil {
