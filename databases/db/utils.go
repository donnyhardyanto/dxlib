...
func DbDriverFormatOrderByFieldName(driverName string, fieldName string, direction string) (string, error) {
	// Format the identifier according to databases requirements
...
func DbDriverConvertValueTypeToDBCompatible(driverName string, v any) (any, error) {
	...
	case []string:
		// Convert to JSON string for cross-databases compatibility (jsonb/json columns)
...
// DBDriverGenerateLimitOffsetClause generates the appropriate LIMIT/OFFSET clause for each databases type
...
// SQLPartWhereAndFieldNameValues generates WHERE clause conditions for different databases types
//
// Parameters:
...
// IsConnectionError checks if the error is a databases connection error
func IsConnectionError(err error) bool {
	...
	// Generic check for all databases drivers based on error message
