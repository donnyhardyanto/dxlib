...
func (t *DXRawTable) Insert(ctx context.Context, l *log.DXLog, data utils.JSON, returningFieldNames []string) (sql.Result, utils.JSON, error) {
...
func (t *DXRawTable) InsertReturningId(ctx context.Context, l *log.DXLog, data utils.JSON) (int64, error) {
	_, returningValues, err := t.Insert(ctx, l, data, []string{t.FieldNameForRowId})
	if err != nil {
		return 0, err
	}
...
func (t *DXRawTable) DoInsert(aepr *api.DXAPIEndPointRequest, data utils.JSON) (int64, error) {
	...
	_, returningValues, err := t.Database.Insert(aepr.Context, t.GetFullTableName(), data, returningFields)
	if err != nil {
		return 0, err
	}
